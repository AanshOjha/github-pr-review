...
def main():
    ...
    # Extract merge recommendation
    merge_rec = review_data.get("mergeRecommendation", {})
    verdict = merge_rec.get("verdict", "COMMENT").upper()
    verdict_reason = merge_rec.get("reason", "")
    ...
    # Map LLM verdict to GitHub review event
    GITHUB_EVENT_MAP = {
        "APPROVE": "APPROVE",
        "REQUEST_CHANGES": "REQUEST_CHANGES",
        "NEEDS_REEVALUATION": "REQUEST_CHANGES",  # still blocks merge
    }
    github_event = GITHUB_EVENT_MAP.get(verdict, "COMMENT")
    ...
    for finding in review_data.get("reviews", []):
        ...
        code_suggestion = finding.get("codeSuggestion")

        if valid_set and l_num in valid_set:
            # Valid inline comment — build body with optional suggestion block
            ...
            note = f"- **[{sev}] (`{f_path}:{l_num}`)**: {comm_text}"
            if code_suggestion:
                note += f"\n  <details><summary>Suggested Fix</summary>\n\n  {code_suggestion}\n\n  </details>"
            general_notes.append(note)
            print(f"  [FALLBACK TO SUMMARY] {f_path}:{l_num} not in diff hunk valid lines.")

    # Construct the summary review body — lead with verdict banner
    if verdict == "APPROVE":
        verdict_banner = f"> {verdict_icon} **VERDICT: SAFE TO MERGE**\n> {verdict_reason}"
    elif verdict == "NEEDS_REEVALUATION":
        verdict_banner = (
            f"> {verdict_icon} **VERDICT: NEEDS REEVALUATION — Do not merge yet**\n"
            f"> {verdict_reason}\n>\n"
            f"> The required corrections span more than ~100 lines. The author should rethink the approach and open a new review request."
        )
    else:  # REQUEST_CHANGES
        verdict_banner = f"> {verdict_icon} **VERDICT: CHANGES REQUESTED — Merge blocked**\n> {verdict_reason}"

    review_body = (
        f"### 🤖 Azure AI Foundry (`{model_name}`) Code Review\n\n"
        f"{verdict_banner}\n\n"
        f"---\n\n"
        f"**Summary:** {summary}\n"
    )
    ...
    # Submit the PR review — use verdict-matched GitHub event
